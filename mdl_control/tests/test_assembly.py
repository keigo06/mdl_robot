...
def test_create_cubic_assembly():
    ...
    assert len(assembly.modules) == 27
    positions = [tuple(cube.pos) for cube in assembly.modules.values()]
    expected_positions = [(
        x*0.12, y*0.12, z * 0.12) for x in range(3) for y in range(3) for z in range(3)]
    assert set(positions) == set(expected_positions)
    ids = [cube.cube_id for cube in assembly.modules.values()]
    assert set(ids) == set(range(27))


def test_get_outermost_module_ids_cubic_assembly():
    assembly = Assembly(num_cubes=27)
    assembly.create_cubic_assembly()
    outermost_ids = assembly.get_outermost_module_ids()
